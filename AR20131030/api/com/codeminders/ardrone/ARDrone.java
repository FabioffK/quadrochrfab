...
public class ARDrone {
	...
	public enum Animation {
		PHI_M30_DEG(0), PHI_30_DEG(1), THETA_M30_DEG(2), THETA_30_DEG(3), THETA_20DEG_YAW_200DEG(
				4), THETA_20DEG_YAW_M200DEG(5), TURNAROUND(6), TURNAROUND_GODOWN(
				7), YAW_SHAKE(8), YAW_DANCE(9), PHI_DANCE(10), THETA_DANCE(11), VZ_DANCE(
				12), WAVE(13), PHI_THETA_MIXED(14), DOUBLE_PHI_THETA_MIXED(15), ANIM_MAYDAY(
				16);
	...
	public enum LED {
		BLINK_GREEN_RED(0), BLINK_GREEN(1), BLINK_RED(2), BLINK_ORANGE(3), SNAKE_GREEN_RED(
				4), FIRE(5), STANDARD(6), RED(7), GREEN(8), RED_SNAKE(9), BLANK(
				10), RIGHT_MISSILE(11), LEFT_MISSILE(12), DOUBLE_MISSILE(13), FRONT_LEFT_GREEN_OTHERS_RED(
				14), FRONT_RIGHT_GREEN_OTHERS_RED(15), REAR_RIGHT_GREEN_OTHERS_RED(
				16), REAR_LEFT_GREEN_OTHERS_RED(17), LEFT_GREEN_RIGHT_RED(18), LEFT_RED_RIGHT_GREEN(
				19), BLINK_STANDARD(20);
	...
	public enum ConfigOption {
		ACCS_OFFSET("control:accs_offset"), ACCS_GAINS("control:accs_gains"), GYROS_OFFSET(
				"control:gyros_offset"), GYROS_GAINS("control:gyros_gains"), GYROS110_OFFSET(
				"control:gyros110_offset"), GYROS110_GAINS(
				"control:gyros110_gains"), GYRO_OFFSET_THR_X(
				"control:gyro_offset_thr_x"), GYRO_OFFSET_THR_Y(
				"control:gyro_offset_thr_y"), GYRO_OFFSET_THR_Z(
				"control:gyro_offset_thr_z"), PWM_REF_GYROS(
				"control:pwm_ref_gyros"), CONTROL_LEVEL("control:control_level"), SHIELD_ENABLE(
				"control:shield_enable"), EULER_ANGLE_MAX(
				"control:euler_angle_max"), ALTITUDE_MAX("control:altitude_max"), ALTITUDE_MIN(
				"control:altitude_min"), CONTROL_TRIM_Z(
				"control:control_trim_z"), CONTROL_IPHONE_TILT(
				"control:control_iphone_tilt"), CONTROL_VZ_MAX(
				"control:control_vz_max"), CONTROL_YAW("control:control_yaw"), OUTDOOR(
				"control:outdoor"), FLIGHT_WITHOUT_SHELL(
				...
				"control:indoor_euler_angle_max"), INDOOR_CONTROL_VZ_MAX(
				"control:indoor_control_vz_max"), INDOOR_CONTROL_YAW(
				"control:indoor_control_yaw"), OUTDOOR_EULER_ANGLE_MAX(
				"control:outdoor_euler_angle_max"), OUTDOOR_CONTROL_VZ_MAX(
				"control:outdoor_control_vz_max"), OUTDOOR_CONTROL_YAW(
				"outdoor_control:control_yaw");
	...
	public final static byte[] DEFAULT_DRONE_IP = { (byte) 192, (byte) 168, (byte) 1,
			(byte) 1 };
	...
	private DroneFTPversionReader versionReader;

	public ARDrone() throws UnknownHostException {
		this(InetAddress.getByAddress(DEFAULT_DRONE_IP), 1000, 1000);
	}

	public ARDrone(InetAddress drone_addr, int navDataReconnectTimeout,
			int videoReconnectTimeout) {
	...
	public void changeToErrorState(Exception ex) {
		synchronized (state_mutex) {
			...
			log.log(Level.FINE, "State changed from " + state + " to "
					+ State.ERROR + " with exception ", ex);
			state = State.ERROR;
			state_mutex.notifyAll();
		}
	}
	...
	public void connect(DataLogger videoLogger, DataLogger navdataLogger)
			throws IOException {
		try {
			...
			try {
				...
				version = Integer.parseInt(versionStr.substring(0,
						versionStr.indexOf('.')));
			} catch (NumberFormatException e) {
				log.log(Level.SEVERE,
						"Failed to discover drone version. Using configuration for drone version: "
								+ version, e);
			}

			cmd_socket = new DatagramSocket();

			cmd_sender = new CommandSender(cmd_queue, this, drone_addr,
					cmd_socket);
			...
			NavDataDecoder nav_data_decoder = (null == ext_nav_data_decoder) ? new ARDrone10NavDataDecoder(
					this, NAVDATA_BUFFER_SIZE) : ext_nav_data_decoder;

			ARDroneDataReader nav_data_reader = (null == navdataLogger) ? new LigthUDPDataReader(
					drone_addr, NAVDATA_PORT, navDataReconnectTimeout)
					: new ARDroneDataReaderAndLogWrapper(
							new LigthUDPDataReader(drone_addr, NAVDATA_PORT,
									navDataReconnectTimeout), navdataLogger);

			drone_nav_channel_processor = new ChannelProcessor(nav_data_reader,
					nav_data_decoder);

			VideoDataDecoder video_data_decoder = (null == ext_video_data_decoder) ? getVideoDecoder(version)
					: ext_video_data_decoder;
			ARDroneDataReader video_data_reader = (null == videoLogger) ? getVideoReader(version)
					: new ARDroneDataReaderAndLogWrapper(new UDPDataReader(
							drone_addr, VIDEO_PORT, videoReconnectTimeout),
							videoLogger);

			if (null != video_data_reader && null != video_data_decoder) {
				drone_video_channel_processor = new ChannelProcessor(
						video_data_reader, video_data_decoder);
			}
	...
	private ARDroneDataReader getVideoReader(int version) throws IOException {
		switch (version) {
		case 1:
			return new LigthUDPDataReader(drone_addr, VIDEO_PORT,
					videoReconnectTimeout);
		case 2:
			return new TCPDataRader(drone_addr, VIDEO_PORT,
					videoReconnectTimeout);
		default:
			return new LigthUDPDataReader(drone_addr, VIDEO_PORT,
					videoReconnectTimeout);
	...
	public void move(float left_right_tilt, float front_back_tilt,
		...
		cmd_queue.add(new MoveCommand(combinedYawMode, left_right_tilt,
				front_back_tilt, vertical_speed, angular_speed));
	}
		...
		try {
			synchronized (state_mutex) {
				...
				if (state == State.TAKING_OFF
						&& nd.getFlyingState() == FlyingState.FLYING) {
					...
					changeState(State.DEMO);
						...
						&& nd.getFlyingState() == FlyingState.LANDED) {
					log.fine("Landing success");
					cmd_queue.clear(); // Maybe we should just remove
										// LAND/TAKEOFF comand
										// instead of nuking the whole queue?
					changeState(State.DEMO);
				} else if (state != State.BOOTSTRAP
						&& nd.getMode() == Mode.BOOTSTRAP) {
					changeState(State.BOOTSTRAP);
				} else if (state == State.BOOTSTRAP
						&& nd.getMode() == Mode.DEMO) {
					changeState(State.DEMO);
				}
	...
	protected void videoFrameReceived(int startX, int startY, int w, int h,
			int[] rgbArray, int offset, int scansize) {
		synchronized (image_listeners) {
			for (DroneVideoListener l : image_listeners)
				l.frameReceived(startX, startY, w, h, rgbArray, offset,
						scansize);
		}
	}
	...
	public void waitForReady(long how_long) throws IOException {
		...
		synchronized (state_mutex) {
			while (true) {
				...
				long p = Math.min(how_long
						- (System.currentTimeMillis() - since), how_long);
				if (p > 0) {
					try {
						state_mutex.wait(p);
	...
	public void setExternalVideoDataDecoder(
			VideoDataDecoder ext_video_data_decoder) {
	...
	public synchronized String readDroneConfiguration() {
		...
		synchronized (this) {
			...
			try {
				...
				log.log(Level.SEVERE,
						"Error. Fialed to read drone configuration", ex);
			} finally {
				try {
					socket.close();
				} catch (IOException e) {
					log.log(Level.SEVERE,
							"Error. Close Drone TCP controll chanel", e);
				}
			}
		}
